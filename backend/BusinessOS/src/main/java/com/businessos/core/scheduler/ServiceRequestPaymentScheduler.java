    ...
    public void cancelUnpaidRequests() {
        ...
        for (ServiceRequest req : requests) {
            if (req.getInvoiceId() != null) {
                ...
                if (invoiceOpt.isPresent() && invoiceOpt.get().getStatus() != InvoiceStatus.PAID) {
                    try {
                        // Using the service method to ensure all status history and notifications are handled
                        // We must set the context to System or bypass security for scheduled jobs if needed.
                        // For simplicity, we just update the entity directly and log history if service method requires current platformuser context.
                        req.setStatus(ServiceRequestStatus.CANCELLED);
                        req.setPermanentlyClosed(true);
