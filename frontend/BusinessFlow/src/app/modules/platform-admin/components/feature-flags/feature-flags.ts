...
export class FeatureFlags implements OnInit {
  ...
  seed(): void {
    ...
    this.flagService.seed().subscribe({
      next: () => { this.seeding = false; this.success = 'Default feature flags seeded'; this.load(); },
      error: (err) => { this.seeding = false; this.error = err?.error?.message || 'Failed to seed flags'; }
    });
  }
